...
void arm_set_height(enum Height height) {
  switch (height) {
    ...
    case ARM_FRONT_CARDBOARD_BOX:
      wb_motor_set_position(arm_elements[ARM2], -1.3);
      wb_motor_set_position(arm_elements[ARM3], 0);
      wb_motor_set_position(arm_elements[ARM4], 1.5);
      wb_motor_set_position(arm_elements[ARM5], 0.0);
      break;
    case ARM_RESET:
      wb_motor_set_position(arm_elements[ARM2], 1.57);
      ...
      break;
    case ARM_BACK_PLATE_HIGH://Low_SecondFloor_ready
      wb_motor_set_position(arm_elements[ARM2], 1);
      wb_motor_set_position(arm_elements[ARM3], -2.2);
      wb_motor_set_position(arm_elements[ARM4], -0.4);
      wb_motor_set_position(arm_elements[ARM5], 0.0);
      break;
    case ARM_BACK_PLATE_LOW://Top_SecondFloor_ready
      wb_motor_set_position(arm_elements[ARM2], 0);
      wb_motor_set_position(arm_elements[ARM3], 0);
      wb_motor_set_position(arm_elements[ARM4], -1.5);
      wb_motor_set_position(arm_elements[ARM5], 0);
      break;
    case ARM_HANOI_PREPARE://Grip_ready
      wb_motor_set_position(arm_elements[ARM2], -1.3);
      wb_motor_set_position(arm_elements[ARM3], -1.8);
      wb_motor_set_position(arm_elements[ARM4], 1.5);//-M_PI_2
      wb_motor_set_position(arm_elements[ARM5], 0);//M_PI_2
      break;
    default:
